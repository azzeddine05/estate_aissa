                        ...
                        <div class="row mb-4">
                            <div class="col-12 col-md-12">
                                <div class="card redial-border-light redial-shadow mb-4">
                                    ...
                                    <div style="font-family:changa;" class="alert alert-danger">
                                        {{ session()->get('error') }}
                                    </div>
                                    @endif
                                   
                                    
                                        <h6 class="header-title pr-3 redial-relative">التقارير </h6>
                                        <table id="example" class="table table-bordered" cellspacing="0" width="100%">
                                            ...
                                            @foreach($reports as $report)
                                                <tr>
                                                    <td>{{$report->name_purchaser}}</td>
                                                    <td>{{$report->nationality_purchaser}}</td>
                                                    <td>{{$report->phone_purchaser}}</td>
                                                    <td>{{$report->type}}</td>
                                                    @if($report->rentorBuy === 'buy')
                                                    {
                                                        <td> للبيع </td>
                                                    }
                                                    @else{
                                                    ...
                                                    <td>{{$report->price}}</td>
                                                    <td>{{$report->remaining_amount}}</td>
                                                    <td>{{$report->submitted_amount}}</td>
                                                        
                                                    <td>
                                                        <div class="btn-group mb-2">
                                                            <a href="#"  title="الإطلاع على التقرير" class="btn btn-secondary"><i class="fa fa-list"></i></a>
                                                        </div>
